...
export default {
  ...
  created() {
    ...
    .registerRenderer('styleRender', (hotInstance, TD, row, col, prop, value, cell) => {
      ...
      if (this.ws && cell.style) {
        const { style: { alignment: { wrapText } = {}, border, fill, font } } = cell;
  ...
  computed: {
    ...
    data() {
      return fixMatrix(
          this.ws.getRows(1, this.ws.actualRowCount).map(row => row._cells.map(item => {
            ...
            if (keys.length) {
              return {
                ...
                ...(keys.reduce((result, key) => {
                  result[key] = {
                    width: 1,
                    color: `#${border.color && indexedColors[border.color.indexed] || border.argb || '000000'}`,
                  }
                  return result;
                }, {})),
              };
  ...
  methods: {
    ...
    alignToClass({ horizontal, vertical }) {
      return [ horizontal, vertical ].filter(i => i).map(key => `ht${key.charAt(0).toUpperCase()}${key.slice(1)}`).join(' ')
    },
    parseTheme() {
      ...
      if (theme) {
        ...
        this.themeColors = [ ...children ].flatMap(node => [ ...node.getElementsByTagName('a:srgbClr') ])
        .map(node => node.getAttribute('val')).filter(i => i);
      }
    },
    // 切换sheet
    handleSheet(index) {
